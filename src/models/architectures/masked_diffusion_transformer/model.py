...
from src.models.architectures.masked_diffusion_transformer.components.diffusion import (
    CosineSchedule,
    DiffusionDenoiser,
)
...
from src.models.architectures.masked_diffusion_transformer.components.trace_encoder import (
    TraceEncoder,
)
from src.models.contracts import GeneratedSuffix, MaskedDiffusionOutput
...
class MaskedDiffusionTransformer(SuffixModel):
    ...
    def forward(self, item: TraceCut) -> MaskedDiffusionOutput:
        ...
        return MaskedDiffusionOutput(
            activity_logits=denoising.activity_logits,
            predicted_noise=denoising.inter_event_noise,
    ...
    def compute_loss(
        self, output: MaskedDiffusionOutput, batch: TraceCut
    ) -> tuple[torch.Tensor, Loss]:
        """Score length, masked activities, and time noise per trace."""
        batch_size = batch.suffix.length.size(dim=0)
        lengths = batch.suffix.length - 1

        length_loss = F.cross_entropy(
            input=output.length_logits, target=lengths - 1, reduction='sum'
        )

        activity_targets = output.activity_targets.masked_fill(
            mask=~output.activity_mask, value=self.fallback_activity_index
        )
        activity_losses = F.cross_entropy(
            input=self._activity_logits(output.activity_logits).transpose(1, 2),
            target=activity_targets,
            reduction='none',
        )
        activity_counts = output.activity_mask.sum(dim=1).clamp(min=1)
        activity_loss = (
            activity_losses.masked_fill(mask=~output.activity_mask, value=0.0).sum(dim=1)
            / activity_counts
        ).sum()

        inter_event_losses = F.mse_loss(
            input=output.predicted_noise,
            target=output.inter_event_noise,
            reduction='none',
        )
        valid_counts = output.valid_positions.sum(dim=1).clamp(min=1)
        inter_event_time_loss = (
            inter_event_losses.masked_fill(mask=~output.valid_positions, value=0.0).sum(dim=1)
            / valid_counts
        ).sum()
    ...
    @torch.no_grad()
    def generate(self, item: TraceCut, *, num_samples: int, sample: bool = True) -> GeneratedSuffix:
        ...
        remaining_times = remaining_time_from_inter_event_times(
            times=inter_event_times, keep=valid, codec=self.codec
        )
