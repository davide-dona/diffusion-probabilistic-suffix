...
class EarlyStopper:
    """
    Stop training once its validation energy score has stopped improving for enough epochs.
    ...
    def update(self, val_score: float, *, epochs_since_last_validation: int) -> bool:
        ...
        if best == float('inf') or best - val_score > abs(best) * self.min_delta_perc:
            ...
            self.counter += epochs_since_last_validation
        ...
        return self.counter >= self.patience_epochs
